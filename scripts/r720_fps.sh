...
run() {  # $1=tag $2..=extra args
  ...
  WAVE_PREFETCH=1 WAVE_FP8_COMPILE=1 WAVE_FP8_SCOPE=all timeout 1400 $PY -m wave_rt \
    --wp-size 5 --rf-step 4 --num-frames $NF --vae-stages 3 --dit-scale 14b --height 720 --width 400 \
...
run R720_base        --kv-context joint  --exchange-mode sync
run R720_sage        --kv-context joint  --exchange-mode sync --attention-backend sa
run R720_sage_w8a8   --kv-context joint  --exchange-mode sync --attention-backend sa --quantization fp8
run R720_full        --kv-context causal --exchange-mode overlap --attention-backend sa --quantization fp8

echo "=== SUMMARY (399 latent / 1596 pixel frames) ==="
$PY - <<'PY'
import json, statistics as s, os
rows=[("R720_base","bf16+SDPA (base)"),("R720_sage","+Sage"),
      ("R720_sage_w8a8","Sage+W8A8(all,cc)"),("R720_full","Sage+W8A8+overlap (full)")]
for tag,lab in rows:
