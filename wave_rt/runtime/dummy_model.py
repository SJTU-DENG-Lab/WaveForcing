...
_PRESETS = {
    "1.3b": dict(
        num_attention_heads=12, attention_head_dim=128, num_layers=30, ffn_dim=8960
    ),
    "5b": dict(
        num_attention_heads=24, attention_head_dim=128, num_layers=30, ffn_dim=13824
    ),
    "14b": dict(
        num_attention_heads=40, attention_head_dim=128, num_layers=40, ffn_dim=13824
    ),
}
...
def install(scale: str, verbose: bool = False) -> None:
    ...
    if not getattr(tl, "_wave_dummy_load_patched", False):
        ...
        def patched_load(
            model_cls, init_params, weight_dir_list, device, param_dtype, **kw
        ):
            ...
            if verbose:
                ...
                print(
                    f"[wave-dummy] built {scale} DiT random-init: "
                    f"{n / 1e9:.2f}B params (weight load skipped)",
                    flush=True,
                )
            return model
