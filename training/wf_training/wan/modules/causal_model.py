...
# Block-causal (WP-compatible). Default is full attn.
# RF_BLOCK_CAUSAL=strict|1|true -> query block i sees history + window blocks <= i.
_RF_BLOCK_MODE = os.environ.get("RF_BLOCK_CAUSAL", "").lower()
_RF_BLOCK_CAUSAL = _RF_BLOCK_MODE not in ("", "0", "false")
# Backend for the block-causal mask: flex_attention BlockMask (default) or dense
# SDPA additive mask (RF_FLEX_ATTN=0, kept as reference/fallback).
_RF_FLEX_ATTN = os.environ.get("RF_FLEX_ATTN", "1").lower() not in ("0", "false")
_RF_USE_FLEX = _RF_BLOCK_CAUSAL and _RF_FLEX_ATTN and FLEX_ATTENTION_AVAILABLE

if _RF_BLOCK_CAUSAL:
    print(f"[rf] block-causal enabled (strict), "
          f"attn backend={'flex_attention' if _RF_USE_FLEX else 'sdpa dense mask'}")


def _block_causal_mask_mod(history_len: int, block_len: int):
    """mask_mod for strict block-causal (Q aligns to last Lq keys)."""
    def mask_mod(b, h, q_idx, kv_idx):
        q_block = q_idx // block_len
        k_block = torch.where(
            kv_idx < history_len,
            torch.full_like(kv_idx, -1),
            (kv_idx - history_len) // block_len,
        )
...
def _block_causal_block_mask(lq: int, lk: int, history_len: int, block_len: int,
                             device):
    """BlockMask for strict block-causal.

    With grad enabled (training), the mask is rebuilt on every call WITHOUT
    caching: non-reentrant gradient checkpointing replays the exact op sequence
    on recompute and a cache hit would desync it (metadata mismatch between
    saved and recomputed tensors). In no-grad contexts (inference) the mask is
    cached per (shape, device) to avoid the rebuild cost.
    """
    assert lk >= history_len + lq, f"lk={lk} history={history_len} lq={lq}"
    if torch.is_grad_enabled():
        return create_block_mask(
            _block_causal_mask_mod(history_len, block_len),
            B=None, H=None, Q_LEN=lq, KV_LEN=lk, device=device)
    key = (lq, lk, history_len, block_len, str(device))
...
def _block_causal_attn_mask(lq: int, lk: int, history_len: int, block_len: int,
                            device, dtype) -> torch.Tensor:
    """Additive mask [1,1,Lq,Lk]: 0=keep, -inf=block. Q aligns to last Lq keys.

    Strict: query block i sees history + window blocks <= i.
    """
    assert lk >= history_len + lq, f"lk={lk} history={history_len} lq={lq}"
    q_idx = torch.arange(lq, device=device)
    k_idx = torch.arange(lk, device=device)
    q_block = q_idx // max(block_len, 1)
    k_win = k_idx - history_len
    k_block = torch.where(
        k_idx < history_len,
        torch.full_like(k_idx, -1),
        k_win // max(block_len, 1),
    )
    allow = k_block.unsqueeze(0) <= q_block.unsqueeze(1)
    mask = torch.zeros(lq, lk, device=device, dtype=dtype)
    mask = mask.masked_fill(~allow, torch.finfo(dtype).min)
    return mask.view(1, 1, lq, lk)
    ...
    def forward(
        ...
        if kv_cache is None:
            ...
            if local_start_index == 0:
                ...
                attn_mask = None
                block_mask = None
                if _RF_BLOCK_CAUSAL and roped_query.shape[1] > self.block_length:
                    if _RF_USE_FLEX:
                        block_mask = _block_causal_block_mask(
                            lq=roped_query.shape[1],
                        ...
                        attn_mask = _block_causal_attn_mask(
                            lq=roped_query.shape[1],
                            lk=roped_key.shape[1],
                            history_len=0,
                            block_len=self.block_length,
                ...
                if updating_cache: # updating working cache with clean frame
                    ...
                    block_mask = None
                    if _RF_BLOCK_CAUSAL:
                        history_len = input_key.shape[1] - roped_query.shape[1]
                        if _RF_USE_FLEX:
                            block_mask = _block_causal_block_mask(
                                lq=roped_query.shape[1],
                                lk=input_key.shape[1],
...
class CausalWanModel(ModelMixin, ConfigMixin):
    ...
    def _set_gradient_checkpointing(self, module=None, value=None, *, enable=None, gradient_checkpointing_func=None):
        if enable is not None:
            self.gradient_checkpointing = bool(enable)
            return
        self.gradient_checkpointing = False if value is None else value
