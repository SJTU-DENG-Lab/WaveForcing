...
class RollingForcingTrainingPipeline:
    ...
    def inference_with_causal_rolling_forcing(
            self,
            noise: torch.Tensor,
        ...
        """Rolling Forcing under block-causal attention (CRF).

        Same rollout as ``inference_with_rolling_forcing``. The caller must keep
        ``causal_model._RF_BLOCK_CAUSAL`` True for the whole forward+backward:
        gradient-checkpoint recomputes read that flag, so do not restore it in
        a try/finally around this call.
        """
        import wf_training.wan.modules.causal_model as causal_model
        assert causal_model._RF_BLOCK_CAUSAL, (
            "inference_with_causal_rolling_forcing requires "
            "_RF_BLOCK_CAUSAL enabled for this step"
        )
        return self.inference_with_rolling_forcing(
            noise=noise,
    ...
    def inference_with_rolling_forcing(
        ...
        # Must be identical across ranks: grad-window count/position determines the
        # number of FSDP forward/backward collectives (student graph).
